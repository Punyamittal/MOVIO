...
const IncidentReportCard: React.FC = () => {
  return (
    <>
      <style jsx global>{`
        :root {
      ...
      <div className="flex flex-col pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full max-w-md min-h-[580px] overflow-hidden transition-colors duration-300">
        ...
        <div className="flex justify-between w-full pl-8 pr-8 mb-4">
          {LEGEND_ITEMS.map((item) => (
            <div key={item.name} className="flex gap-2 items-center">
              ...
              <span className="text-gray-500 dark:text-gray-400 text-xs transition-colors duration-300">
                {item.name}
              </span>
            </div>
          ))}
        </div>
        <div className="reaviz-chart-container h-[200px]">
          <AreaChart
            height={200}
        ...
        <div className="flex flex-col pl-8 pr-8 pt-8 font-mono divide-y divide-gray-200 dark:divide-gray-700 transition-colors duration-300">
          {METRICS_DATA.map((metric) => (
            ...
            >
              ...
              <div className="flex gap-2 w-1/2 justify-end items-center">
                <span className="font-semibold text-xl text-black dark:text-white transition-colors duration-300">
                  {metric.value}
                </span>
                <metric.TrendIcon
